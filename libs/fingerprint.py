...
def fingerprint(channel_samples, Fs=DEFAULT_FS,
    ...
    print(colored(msg, attrs=['dark']) % len(local_maxima)) # L'ERREUR C'ETAIT QUE L'OBJET LOCAL_MAXIMA N'AVAIT PAS DE LEN() VOIR LA FONCTION get_2D_peaks()
    ...
    for i in range(len(peaks)):
      for j in range(1, fan_value):
        if (i + j) < len(peaks):
          ...
          t_delta = t2 - t1
          print(freq1, freq2, t_delta)
          # check if delta is between min & max
          if t_delta >= MIN_HASH_TIME_DELTA and t_delta <= MAX_HASH_TIME_DELTA:
            # b = hashlib.sha1("%s|%s|%s" % (str(freq1), str(freq2), str(t_delta)))
            h = hashlib.sha1(str("%s|%s|%s").encode('utf-8') % (freq1, freq2, t_delta)) # encodé les valeurs avant de les hashé
